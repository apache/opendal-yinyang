...
struct ProviderTests {
  static func main() throws {
    ...
    for name in ["a", "b", "c"] {
      files.append(
        try mount.item(["op": "create", "parent": folder.node, "name": name, "directory": false]))
    }
    let nested = try mount.item([
      "op": "create", "parent": folder.node, "name": "nested", "directory": true,
    ])
    let leaf = try mount.item([
      "op": "create", "parent": nested.node, "name": "leaf", "directory": false,
    ])
    config["staging"] = staging.appendingPathComponent("sync").path
    let sync = try NativeBridge(config: config, mode: "sync")
    let domain = NSFileProviderDomain(
      identifier: NSFileProviderDomainIdentifier(UUID().uuidString),
      displayName: "Unregistered callback test")
    ...
    firstEnumerator.enumerateItems(
      for: first, startingAt: NSFileProviderPage(NSFileProviderPage.initialPageSortedByName as Data)
    )
    try first.wait()
    ...
    while let token = page {
      ...
      try observer.wait()
      names += observer.items.map(\.filename)
      page = observer.next
    ...
    while let token = page {
      ...
      for item in observer.items {
        precondition(identities.insert(item.itemIdentifier.rawValue).inserted)
      }
      page = observer.next
    }
    ...
    func modify(expectError: Bool) {
      let done = DispatchSemaphore(value: 0)
      var failure: Error?
      _ = adapter.modifyItem(
        item, baseVersion: item.itemVersion, changedFields: .contents, contents: upload,
        options: [], request: NSFileProviderRequest()
      ) { result, fields, pending, error in
        failure = error
    ...
    let bytes = try sync.read(
      node: accepted.node, offset: 0, length: 64, revision: accepted.revision)
