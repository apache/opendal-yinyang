...
final class ProviderEnumerator: NSObject, NSFileProviderEnumerator {
    ...
    func invalidate() {
        session.queue.async {
            ...
            if count == 0 { self.session.enumerators.removeValue(forKey: self.container) }
            else { self.session.enumerators[self.container] = count }
        }
    }
    private func decode(_ data: Data) throws -> ProviderCursor {
        guard data.count <= 500,
              ...
              cursor.container == container.rawValue, cursor.offset >= 0, cursor.offset <= 1_000_000 else {
    ...
    private func current() throws -> ProviderCursor {
        ...
        return ProviderCursor(domain: session.domain.identifier.rawValue,
    ...
    func currentSyncAnchor(completionHandler: @escaping (NSFileProviderSyncAnchor?) -> Void) {
        session.queue.async {
            guard !self.invalidated, let cursor = try? self.current(),
                  let data = try? JSONEncoder().encode(cursor) else { return completionHandler(nil) }
            ...
            do {
                ...
                if page.rawValue == NSFileProviderPage.initialPageSortedByDate as Data || page.rawValue == NSFileProviderPage.initialPageSortedByName as Data {
                    ...
                    if !item.metadata.directory {
                        observer.didEnumerate([item]); observer.finishEnumerating(upTo: nil); return
                    }
                } else { cursor = try self.decode(page.rawValue) }
                ...
                walk: while directoryIndex < directories.count {
                    ...
                    var offset = 0
                    while true {
                        let result = try bridge.call(["op": "scan", "node": directory, "revision": cursor.revision,
                                                      "offset": offset, "limit": 256])
                        guard let entries = result["entries"] as? [[String: Any]] else { throw POSIXError(.EIO) }
                        for entry in entries {
                            let item = ProviderItem(try NativeItem(entry), root: self.session.root!)
                            if self.container == .workingSet && item.metadata.directory { directories.append(item.metadata.node) }
    ...
    func enumerateChanges(for observer: NSFileProviderChangeObserver, from anchor: NSFileProviderSyncAnchor) {
        session.queue.async {
            do {
                ...
                let result = try bridge.call(["op": "changes", "revision": cursor.revision,
                                              "ordinal": cursor.ordinal, "limit": 1])
                guard let changes = result["changes"] as? [[String: Any]], let revision = result["revision"] as? String,
                      let ordinal = result["ordinal"] as? UInt32, let more = result["more"] as? Bool else { throw POSIXError(.EIO) }
                ...
                let end = min(changes.count, cursor.offset + min(256, max(1, observer.suggestedBatchSize ?? 256)))
                for change in changes[cursor.offset..<end] {
                    ...
                    if let after, included(after) {
                        ...
                        deletions.insert(before.node); updates.removeValue(forKey: before.node)
                    }
                }
                let partial = end < changes.count
                if partial { cursor.offset = end }
                else { cursor = ProviderCursor(domain: cursor.domain, container: cursor.container, revision: revision, ordinal: ordinal) }
                observer.didDeleteItems(withIdentifiers: deletions.map { $0 == self.session.root! ? .rootContainer : NSFileProviderItemIdentifier($0) })
                observer.didUpdate(Array(updates.values))
                observer.finishEnumeratingChanges(upTo: NSFileProviderSyncAnchor(try JSONEncoder().encode(cursor)), moreComing: partial || more)
            } catch { observer.finishEnumeratingWithError(self.enumerationError(error)) }
        }
    }
    private func enumerationError(_ error: Error) -> Error {
        if let native = error as? NativeFailure, native.kind == "Invalid" { return NSFileProviderError(.syncAnchorExpired) }
