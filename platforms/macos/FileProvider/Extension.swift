...
final class ProviderSession {
    ...
    func invalidate() {
        queue.async { self.polling?.cancel(); self.polling = nil; self.bridge = nil }
    }
    func connected() throws -> NativeBridge {
        if let bridge { return bridge }
        guard let directory = FileManager.default.containerURL(forSecurityApplicationGroupIdentifier: providerGroup) else { throw NSFileProviderError(.notAuthenticated) }
...
final class FileProviderExtension: NSObject, NSFileProviderReplicatedExtension, NSFileProviderPartialContentFetching {
    ...
    private func fetch(_ identifier: NSFileProviderItemIdentifier, version: NSFileProviderItemVersion?,
                       range: NSRange?, alignment: Int,
                       completion: @escaping (URL?, NSFileProviderItem?, NSRange, Error?) -> Void) -> Progress {
        let progress = Progress(totalUnitCount: 1)
        session.queue.async {
            var temporary: URL?
            do {
                ...
                if version != nil && revision == nil { throw NSFileProviderError(.versionNoLongerAvailable) }
                let item = try self.session.item(identifier, revision: revision)
                guard !item.metadata.directory, item.metadata.size <= UInt64(Int.max) else { throw POSIXError(.EFBIG) }
                let size = Int(item.metadata.size)
                let lower: Int
                let upper: Int
                if let range {
                    guard alignment > 0, range.location != NSNotFound, range.location <= size,
                          range.length <= size - range.location else { throw POSIXError(.EINVAL) }
                ...
                } else { lower = 0; upper = size }
                guard let manager = NSFileProviderManager(for: self.session.domain) else { throw NSFileProviderError(.noSuchItem) }
                ...
                while position < upper {
                    ...
                    let bytes = try bridge.read(node: item.metadata.node, offset: UInt64(position), length: count, revision: item.metadata.revision)
    ...
    func fetchContents(for itemIdentifier: NSFileProviderItemIdentifier, version requestedVersion: NSFileProviderItemVersion?,
        ...
        fetch(itemIdentifier, version: requestedVersion, range: nil, alignment: 1) { url, item, _, error in completionHandler(url, item, error) }
    }
    func fetchPartialContents(for itemIdentifier: NSFileProviderItemIdentifier, version requestedVersion: NSFileProviderItemVersion,
        ...
        fetch(itemIdentifier, version: requestedVersion, range: minimalRange, alignment: aligningTo) { url, item, range, error in completionHandler(url, item, range, [], error) }
    }
        ...
        session.queue.async {
            do {
                guard changedFields.contains(.contents), let url = newContents,
                      let revision = String(data: version.contentVersion, encoding: .utf8) else { throw CocoaError(.featureUnsupported) }
                if progress.isCancelled { throw CocoaError(.userCancelled) }
                let bridge = try self.session.connected()
                let edit = try bridge.call(["op": "stage", "node": try self.session.identity(item.itemIdentifier), "revision": revision, "path": url.path])
                if progress.isCancelled { throw CocoaError(.userCancelled) }
    ...
    func createItem(basedOn itemTemplate: NSFileProviderItem, fields: NSFileProviderItemFields, contents url: URL?,
                    ...
                    completionHandler: @escaping (NSFileProviderItem?, NSFileProviderItemFields, Bool, Error?) -> Void) -> Progress {
        completionHandler(nil, fields, false, CocoaError(.featureUnsupported)); return Progress()
    }
    func deleteItem(identifier: NSFileProviderItemIdentifier, baseVersion: NSFileProviderItemVersion,
                    options: NSFileProviderDeleteItemOptions, request: NSFileProviderRequest,
                    completionHandler: @escaping (Error?) -> Void) -> Progress {
        completionHandler(CocoaError(.featureUnsupported)); return Progress()
    }
