...
// under the License.
  ...
  final class ProviderDomains: ObservableObject {
    ...
    func add() {
      ...
      picker.message =
        "Choose an isolated test resource containing volume.json. Its credentials will be copied into this prototype's App Group."
      ...
      do {
        guard
          let group = FileManager.default.containerURL(
            forSecurityApplicationGroupIdentifier: "group.com.xuanwo.yinyang.prototype")
        ...
        guard var config = try JSONSerialization.jsonObject(with: data) as? [String: Any],
          ...
          ["minio", "amazon-s3"].contains(profile), config["read_only"] as? Bool != true
        ...
        guard
          FileManager.default.createFile(
            atPath: directory.appendingPathComponent("volume.json").path, contents: output,
        ...
        let domain = NSFileProviderDomain(
          ...
          displayName: "YinYang Prototype \(id.prefix(8))")
        busy = true
        NSFileProviderManager.add(domain) { error in
          Task { @MainActor in
            ...
            self.status =
              error.map {
                "Domain registration failed: \($0.localizedDescription). Configuration retained."
              } ?? "Registered isolated domain \(id)."
          }
        }
    ...
    func refresh() {
      ...
      NSFileProviderManager.getDomainsWithCompletionHandler { domains, error in
        ...
        Task { @MainActor in
          self.busy = false
          self.status =
            error?.localizedDescription ?? "Refresh requested for this prototype's domains."
        }
      }
    }
    func remove() {
      ...
      alert.informativeText =
        "Only UUIDs registered by this app are affected. The OS may remove downloaded test copies. App Group staging is retained for recovery; export pending edits before removal."
      ...
      NSFileProviderManager.getDomainsWithCompletionHandler { domains, error in
        ...
        group.notify(queue: .main) {
          Task { @MainActor in
            ...
            self.status =
              failures.isEmpty
              ? "Test domains removed; staging retained." : failures.joined(separator: "\n")
          }
        }
      }
    }
